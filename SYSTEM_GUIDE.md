# 🕶️ Как превратить IDA в Системного помощника на твоем ПК
...
## 2. Управление системой через `ida_system.py`

Я создал файл `ida_system.py` в корне твоего проекта. Он расширяет стандартную IDA и добавляет команды:
- **"Открой [название]"** — открывает программы (браузер, блокнот, игры).
- **"Найди в интернете [запрос]"** — открывает поиск в браузере.

### Как запустить:
```bash
python ida_system.py
```
...
Чтобы IDA не слушал всё подряд, а реагировал только на "IDA" или "Ида", используй библиотеку `pvporcupine`.
...
## 4. Планы по интеграции "Системного помощника"
